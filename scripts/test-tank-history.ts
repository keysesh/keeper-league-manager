  ...
  for (let i = 0; i < timeline.length; i++) {
    ...
    for (let j = i + 1; j < timeline.length; j++) {
      ...
      if (next.date) {
        ...
        if (daysDiff <= 1) {
          console.log("FILTERING: " + current.date?.split("T")[0] + " DROP by " + current.teamName + " -> " + next.date?.split("T")[0] + " DRAFT by " + next.teamName + " (" + daysDiff.toFixed(2) + " days)");
...
async function main() {
  console.log("=== TESTING TANK BIGSBY HISTORY ===\n");

  const player = await prisma.player.findFirst({
    where: { fullName: { contains: "Tank Bigsby" } },
  });
  ...
  console.log("Player: " + player.fullName + " (Sleeper: " + player.sleeperId + ")\n");
  ...
  console.log("=== BEFORE FILTER (" + timeline.length + " events) ===");
  for (const e of timeline) {
    const dateStr = e.date ? new Date(e.date).toISOString().split("T")[0] : "no-date";
    console.log(e.season + " " + dateStr + " " + e.event.padEnd(12) + " " + e.teamName.padEnd(25) + " [" + e.leagueName + "]");
  }

  const filtered = filterGlitchTransactions(timeline);

  console.log("\n=== AFTER FILTER (" + filtered.length + " events) ===");
